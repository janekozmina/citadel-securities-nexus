...
type RvpFormData = z.infer<typeof rvpSchema>;

interface RvpInstructionFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function RvpInstructionForm({ open, onOpenChange }: RvpInstructionFormProps) {
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const form = useForm<RvpFormData>({
    resolver: zodResolver(rvpSchema),
    defaultValues: {
      reference: `RVP-${Date.now()}`,
  ...
  const onSubmit = async (data: RvpFormData) => {
    setIsSubmitting(true);
    try {
      console.log('RvP Instruction submitted:', data);
      toast({
        title: "Success",
        description: "RvP instruction has been created successfully.",
      });
      onOpenChange(false);
      form.reset();
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to create RvP instruction. Please try again.",
        variant: "destructive",
      });
    } finally {
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            RvP Instruction (MT541) - Receive against Payment
          </DialogTitle>
        </DialogHeader>
