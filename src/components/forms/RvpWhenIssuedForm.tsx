...
const rvpWhenIssuedSchema = z.object({
...
type RvpWhenIssuedFormData = z.infer<typeof rvpWhenIssuedSchema>;

interface RvpWhenIssuedFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function RvpWhenIssuedForm({ open, onOpenChange }: RvpWhenIssuedFormProps) {
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const form = useForm<RvpWhenIssuedFormData>({
    resolver: zodResolver(rvpWhenIssuedSchema),
    defaultValues: {
      reference: `RVPWI-${Date.now()}`,
  ...
  const onSubmit = async (data: RvpWhenIssuedFormData) => {
    setIsSubmitting(true);
    try {
      console.log('RvP When-Issued Instruction submitted:', data);
      toast({
        title: "Success",
        description: "RvP (When-Issued) instruction has been created successfully.",
      });
      onOpenChange(false);
      form.reset();
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to create RvP (When-Issued) instruction. Please try again.",
        variant: "destructive",
      });
    } finally {
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            RvP When-Issued (MT541) - Receive against Payment (When Issue)
          </DialogTitle>
        </DialogHeader>
