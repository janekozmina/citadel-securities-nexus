...
const islamicRepoDvpSchema = z.object({
...
type IslamicRepoDvpFormData = z.infer<typeof islamicRepoDvpSchema>;

interface IslamicRepoDvpFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function IslamicRepoDvpForm({ open, onOpenChange }: IslamicRepoDvpFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const form = useForm<IslamicRepoDvpFormData>({
    resolver: zodResolver(islamicRepoDvpSchema),
    defaultValues: {
      reference: 'FIBHSER10113S014',
      dealPrice: '0.000',
      faceAmount: '0.000',
      repoRate: '0.00',
  ...
  const onSubmit = async (data: IslamicRepoDvpFormData) => {
    setIsSubmitting(true);
    try {
      console.log('Islamic Repo DvP Form Data:', data);
      const transactionId = generateTransactionId();
      toast.success(`Created Transaction ${transactionId}`);
      onOpenChange(false);
      form.reset();
    } catch (error) {
      toast.error('Failed to create Islamic Repo DvP transaction');
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>New MT543 islamic REPO</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Delivering agent</CardTitle>
              </CardHeader>
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  control={form.control}
            ...
            <Card>
              ...
              <CardContent>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Trade regulator*</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder="Select trade regulator" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value="reg1">Regulator 1</SelectItem>
                          <SelectItem value="reg2">Regulator 2</SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage />
                    </FormItem>
