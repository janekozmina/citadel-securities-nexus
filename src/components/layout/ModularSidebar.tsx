                ...
                >
                  {hasChildren ? (
                    <div className="flex items-center gap-2 w-full">
                      ...
                      {!isCollapsed && (
                        <span className="font-medium text-[14px] leading-tight">{getHighlightedText(item.title, searchQuery)}</span>
                      )}
                    </div>
                  ) : (
      ...
      <div className="h-full flex flex-col bg-black/20">
        ...
        <div className="px-3 py-2 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-xs text-white truncate">{selectedMainItem.title}</span>
          </div>
        </div>
                      ...
                      {hasChildren ? (
                        <div className="flex items-center justify-between w-full min-w-0">
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <span className="font-medium text-[12px] leading-tight">{getHighlightedText(subItem.title, searchQuery)}</span>
                          </div>
                          ...
                          <SidebarMenuItem key={thirdItem.id}>
                            <SidebarMenuButton asChild>
                              ...
                              >
                                ...
                                <span className="font-medium text-[11px] leading-tight">{thirdItem.title}</span>
                              </NavLink>
                            </SidebarMenuButton>
                          </SidebarMenuItem>
                        );
  ...
  return (
    <Sidebar
      className={cn(
        ...
        isCollapsed ? "w-16" : selectedMainItem?.children ? "w-[320px]" : "w-52"
      )}
      style={{
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <div className="flex h-full">
              ...
              {/* Sub navigation panel - Compact width for text visibility */}
              {selectedMainItem?.children && !isCollapsed && (
                <div className="w-64 border-l border-white/10">
                  {renderSubPanel()}
                </div>
              )}
            </div>
