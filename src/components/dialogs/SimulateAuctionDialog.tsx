...
const SimulateAuctionDialog: React.FC<SimulateAuctionDialogProps> = ({ 
  ...
  const [quantityToSatisfy, setQuantityToSatisfy] = useState("50000000");
  const [useQuantity, setUseQuantity] = useState(true);
  const [cutOffPrice, setCutOffPrice] = useState("5.50000");
  ...
  const runSimulation = () => {
    // Parse input values
    const quantity = parseFloat(quantityToSatisfy.replace(/,/g, '')) || 50000000;
    const price = parseFloat(cutOffPrice) || 5.50000;
    
    // Generate simulation results based on input values
    ...
    // Calculate values based on quantity input
    const satisfiedValue = Math.floor(quantity * 0.6 + Math.random() * quantity * 0.2);
    const partlySatisfiedValue = Math.floor(quantity * 0.2 + Math.random() * quantity * 0.1);
    const rejectedValue = Math.floor(quantity * 0.15 + Math.random() * quantity * 0.1);
    const partlyRejectedValue = Math.floor(quantity * 0.05 + Math.random() * quantity * 0.05);
    
    // Use the cut off price as base for winner price
    const winnerPrice = price + (Math.random() - 0.5) * 0.5;
    ...
    setSimulationResults({
      ...
      satisfiedValue,
      partlySatisfiedValue,
      rejectedValue,
      partlyRejectedValue,
      winnerPrice: Math.max(winnerPrice, 0),
      minPrice: Math.max(price - 0.5, 0),
      maxPrice: price + 0.5
    });
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Input
                      type="text"
                      value={quantityToSatisfy}
                      onChange={(e) => setQuantityToSatisfy(e.target.value)}
                    ...
                    <div className="flex items-center gap-1">
                      <Checkbox 
                        id="use-quantity" 
                        checked={useQuantity}
                        onCheckedChange={(checked) => setUseQuantity(checked as boolean)}
                      />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      <Checkbox 
                        id="use-cutoff" 
                        checked={useCutOff}
                        onCheckedChange={(checked) => setUseCutOff(checked as boolean)}
                      />
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span>{((simulationResults.minPrice + simulationResults.maxPrice) / 2).toFixed(5)}</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
