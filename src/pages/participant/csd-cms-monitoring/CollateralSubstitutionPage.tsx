...
const CollateralSubstitutionPage = () => {
  const [repoTransaction, setRepoTransaction] = useState('CITIS0011126S003');
  const [selectedInstrument, setSelectedInstrument] = useState('');
  const [totalRequested, setTotalRequested] = useState('100,000.00');
  const [totalChosen, setTotalChosen] = useState('100,000.00');

  const collateralData = [
    {
      instrumentCode: 'FR001',
      chosenQuantity: 1,
      availableQuantity: 26937.00,
      availableCollateralValue: 2693700000.00,
      marketPrice: 100,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Collateral Substitution</h1>
        <p className="text-muted-foreground">Manage collateral substitution for repo transactions</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Collateral Substitution Details</CardTitle>
        </CardHeader>
...
export default CollateralSubstitutionPage;